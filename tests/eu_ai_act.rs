...
fn eu_ai_act_structure() {
    ...
    match &art3_alineas[0].content[0] {
        Subparagraph::List(lb) => {
            assert_eq!(
                lb.items.len(),
                68,
                "Article 3 list should have 68 definition items"
            );
            assert!(
                matches!(
                    &lb.items[0],
    ...
    assert_eq!(
        ch1_art5.number, "Article 5",
        "unexpected article at index 0 of Chapter II"
    );
    let ArticleContent::Paragraphs(ref art5_paras) = ch1_art5.content else {
        panic!("Article 5 should have Paragraphs content");
    };
    let para1 = &art5_paras[0];
    assert_eq!(para1.number, "1.");
    // 2 <ALINEA> elements: first has the List block, second has trailing plain Text
    assert_eq!(
        para1.alineas.len(),
        2,
            ...
            match &lb.items[2] {
                Item {
                    ...
                    assert_eq!(
                        inner.items.len(),
                        2,
                        "Article 5 item (c) should have 2 sub-items"
                    );
                }
            ...
            match &lb.items[7] {
                Item {
                    ...
                    assert_eq!(
                        inner.items.len(),
                        3,
                        "Article 5 item (h) should have 3 sub-items"
                    );
                }
    ...
    assert!(
        annex_iii.number.contains("III"),
        "expected ANNEX III at index 2"
    );
    ...
    assert_eq!(
        iii_list.items.len(),
        8,
        "Annex III should have 8 high-risk category items"
    );
    ...
    match &iii_list.items[0] {
        Item {
            ...
            inner.items.len(),
            3,
            "Annex III item 1 should have 3 sub-items"
        ),
    ...
    assert!(
        annex_iv.number.contains("IV"),
        "expected ANNEX IV at index 3"
    );
    ...
    assert!(
        ...
        "Annex IV has {} list item(s) with empty text",
        empty_items.len()
    );
    ...
    assert_eq!(
        reg.definitions.len(),
        68,
        "EU AI Act should have 68 definitions"
    );
    assert!(
...
fn eu_ai_act_recital_citations() {
    ...
    assert!(
        r10.contains(&Citation {
            act_type: CitedActType::Regulation,
            regime: Some("EU".into()),
            number: "2016/679".into(),
            oj_ref: Some(OjRef {
                collection: "L".into(),
                number: "119".into(),
                date: "20160504".into(),
    ...
    assert!(
        r11.contains(&Citation {
            act_type: CitedActType::Regulation,
            regime: Some("EU".into()),
            number: "2022/2065".into(),
            oj_ref: Some(OjRef {
                collection: "L".into(),
                number: "277".into(),
                date: "20221027".into(),
    ...
    for c in r14 {
        assert!(
            c.oj_ref.is_none(),
            "recital (14): all citations should be inline (no OJ ref), found {:?}",
            c
        );
    }
}
...
fn eu_ai_act_metadata() {
    ...
    assert!(
        md.authors.is_empty(),
        "EU AI Act BIB.DOC has no AUTHOR elements"
    );
